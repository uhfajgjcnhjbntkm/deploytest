...
Запуск:		(b=main cmd=( deploy.sh -c 'https://disk.yandex.ru/d/1dlKI8SikxkEBA' -z ) ;curl -sfOL "https://raw.githubusercontent.com/uhfajgjcnhjbntkm/deploytest/$b/${cmd[0]}"&&{ chmod +x ${cmd[0]}&&./"${cmd[@]}";rm -f ${cmd[0]};:;}||echo -e "\e[1;33m\nОшибка скачивания: проверьте подключение к Интернету, настройки DNS, прокси и URL адрес\ncurl exit code: $?\n\e[m">&2)
